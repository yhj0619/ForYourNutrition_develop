	...
	public void addCartItem(Member member, CartItem cartItem) {
		try {
            ...
            for (CartItem itemInCart : cart.getCartItems()) {
                totalQuantity += itemInCart.getQuantity(); // 각 cartItem의 quantity를 하나씩 더함
            }

            cart.setQuantity(totalQuantity); // 합친 값을 cart의 quantity로 설정
            cartDao.saveCart(cart); // 변경된 cart를 저장
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
